...
export class WhatsAppSyncStore {
  ...
  constructor(readonly home: string) {
    ...
    this.ensureImportFromColumn();
  }

  /** Additive column for existing sqlite DBs — never resets a stored cutoff. */
  private ensureImportFromColumn(): void {
    const cols = this.db.prepare(`PRAGMA table_info(sync_state)`).all() as Array<{ name: string }>;
    if (!cols.some((col) => col.name === "import_from_at")) {
  ...
  getCursor(conversationId: string): { lastMessageId: string | null; lastTimestamp: number | null } {
    const row = this.db
      .prepare(`SELECT last_message_id, last_timestamp FROM conversation_cursors WHERE conversation_id = ?`)
      .get(conversationId) as { last_message_id: string | null; last_timestamp: number | null } | undefined;
    return {
  ...
  upsertCursor(input: {
    ...
    this.db
      .prepare(
        `INSERT INTO conversation_cursors (conversation_id, last_message_id, last_timestamp, imported_count, updated_at)
         VALUES (?, ?, ?, ?, ?)
      ...
      .run(input.conversationId, input.lastMessageId, input.lastTimestamp, input.importedDelta, input.at);
  }

  hasImportedKey(sourceKey: string): boolean {
    const row = this.db.prepare(`SELECT 1 AS ok FROM imported_keys WHERE source_key = ?`).get(sourceKey) as
      | { ok: number }
      | undefined;
    return Boolean(row);
  }

  markImportedKeys(keys: Array<{ sourceKey: string; conversationId: string }>, at: string): void {
    const stmt = this.db.prepare(
      `INSERT OR IGNORE INTO imported_keys (source_key, conversation_id, imported_at) VALUES (?, ?, ?)`,
    );
    for (const row of keys) {
      stmt.run(row.sourceKey, row.conversationId, at);
    }
  }
