...
export const JOBS_TABLE = "whatsapp_sync_jobs";
  ...
  return {
    async enqueueManual(userId: string, now: Date = new Date()): Promise<EnqueueResult> {
      return enqueueIfIdle(store, {
        ...
        requestedBy: userId,
        now,
        conflictMessage: "Could not enqueue a manual WhatsApp sync job.",
      });
    },
    ...
    async enqueueTriggered(
      ...
      return enqueueIfIdle(store, {
        ...
        requestedBy: null,
        now,
        conflictMessage: `Could not enqueue a ${trigger} WhatsApp sync job.`,
      });
    },
...
export async function enqueueManualForUser(queue: JobQueuePort, userId: string | null, now: Date = new Date()) {
  if (!userId) {
    throw new JobQueueError("Sign in to queue a WhatsApp sync.");
  }
...
export async function readStatusForUser(queue: JobQueuePort, userId: string | null) {
  if (!userId) {
    throw new JobQueueError("Sign in to view WhatsApp sync status.");
  }
