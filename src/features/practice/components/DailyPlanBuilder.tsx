...
const field = "h-10 w-full rounded-control border border-border bg-white px-3 text-sm outline-none focus:border-[var(--module-accent)]";

function signed(value: number) { return value > 0 ? `+${value}` : String(value); }

function PlanDayRuleSummary({ plan, summary }: { plan: DailyPracticePlan; summary: DayRuleSummary }) {
  const month = Number(plan.planDate.slice(5, 7));
  const year = Number(plan.planDate.slice(0, 4));
  const day = Number(plan.planDate.slice(8, 10));
  const rowIndex = summary.rows.findIndex((row) => row.month === month);
  const row = summary.rows[rowIndex];
  const counted = row?.days.some((item) => item.date === plan.planDate) ?? plan.countable;
  const daysInMonth = new Date(year, month, 0).getDate();
  const budgetToPlanDate = rowIndex < 0 ? 0 : summary.rows.slice(0, rowIndex).reduce((sum, item) => sum + item.budget, 0) + Math.round(row.budget * day / daysInMonth);
  const seasonStartYear = month >= 8 ? year : year - 1;
  const seasonStart = `${seasonStartYear}-08-01`;
  const usedToPlanDate = new Set(summary.rows.flatMap((item) => item.days.map((countedDay) => countedDay.date)).filter((date) => date >= seasonStart && date <= plan.planDate)).size;
  const toDateVariance = budgetToPlanDate - usedToPlanDate;
  const monthVariance = row ? row.budget - row.used : 0;
  const seasonUsed = summary.used;
  return <div className="mb-4 grid gap-2" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(135px, 1fr))" }}>
    <Metric label="This date" value={counted ? "Counted" : "Not counted"} detail={counted ? "1 day toward 114" : "0 days toward 114"} tone={counted ? "green" : "slate"}/>
    <Metric label={row?.label ?? "Month"} value={`${row?.used ?? 0} / ${row?.budget ?? 0}`} detail={`${signed(monthVariance)} to monthly budget`} tone={monthVariance < 0 ? "red" : "blue"}/>
    <Metric label="Season total" value={`${seasonUsed} / ${summary.limit}`} detail={`${summary.remaining} days remaining`} tone={summary.remaining < 0 ? "red" : "violet"}/>
    <Metric label="Budget to this date" value={`${usedToPlanDate} / ${budgetToPlanDate}`} detail={`${signed(toDateVariance)} to date · positive is under`} tone={toDateVariance < 0 ? "red" : "green"}/>
  </div>;
}

function Metric({ label, value, detail, tone }: { label: string; value: string; detail: string; tone: "green" | "blue" | "violet" | "red" | "slate" }) {
  const colors = { green: "border-emerald-200 bg-emerald-50 text-emerald-900", blue: "border-blue-200 bg-blue-50 text-blue-900", violet: "border-violet-200 bg-violet-50 text-violet-900", red: "border-red-200 bg-red-50 text-red-900", slate: "border-slate-200 bg-slate-50 text-slate-700" };
  return <div className={`rounded-control border p-3 ${colors[tone]}`}><p className="text-[9px] font-bold tracking-wider uppercase opacity-70">{label}</p><p className="mt-1 text-lg font-bold tabular-nums">{value}</p><p className="mt-0.5 text-[10px] font-medium opacity-75">{detail}</p></div>;
}
...
export default function DailyPlanBuilder({ drills, plans, dayRule, initialPlanId = null, initialPlanDate = null }: { drills: PracticeDrill[]; plans: DailyPracticePlan[]; dayRule: DayRuleSummary; initialPlanId?: string | null; initialPlanDate?: string | null }) {
  ...
  if (!editing && current) return <div className="grid gap-3">
    {activePlanId ? <button type="button" onClick={backToNewPlan} className="inline-flex w-fit shrink-0 items-center gap-1.5 text-sm font-medium text-text-secondary transition-colors duration-150 hover:text-text-primary"><ArrowLeft className="h-4 w-4" strokeWidth={1.75}/>Back to New Plan</button> : null}
    <div className={styles.savedLayout}>
    <section className="overflow-hidden rounded-card border border-violet-200 bg-surface shadow-[0_12px_30px_rgba(124,58,237,0.08)]"><div className="p-4 text-white" style={{ background: "linear-gradient(90deg, #6d28d9, #4f46e5)" }}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] font-semibold tracking-wider text-violet-100 uppercase">{current.planDate === today ? "Today’s Plan" : "Previous Plan"} · {current.planDate}</p><h2 className="mt-1 text-xl font-semibold">{current.title}</h2><p className="mt-1 text-xs text-violet-100">{current.startTime || "Time TBD"}{current.endTime ? `–${current.endTime}` : ""} · {current.location || "Location TBD"}</p></div><button type="button" onClick={() => beginEditing(current)} className="inline-flex h-9 items-center gap-1.5 rounded-control bg-white px-3 text-xs font-semibold text-violet-700"><Pencil className="h-3.5 w-3.5"/>Edit Plan</button></div></div><div className="p-5"><PlanDayRuleSummary plan={current} summary={dayRule}/><div className="mb-4 rounded-control border border-amber-200 bg-amber-50 p-3"><div className="flex items-center gap-2 text-xs font-semibold text-amber-900"><BellRing className="h-4 w-4"/>Team Announcements</div><p className="mt-1.5 whitespace-pre-wrap text-sm text-amber-950">{current.announcements || "No announcements."}</p></div><h3 className="text-xs font-semibold tracking-wide text-text-secondary uppercase">Practice sequence</h3><div className="mt-2 space-y-2">{current.drills.length ? current.drills.map((drill, index) => <div key={drill.id} className="flex items-start gap-3 rounded-control border border-border p-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-violet-100 text-xs font-bold text-violet-700">{index + 1}</span><div><p className="text-sm font-semibold">{drill.name}</p><p className="mt-0.5 text-xs text-text-secondary">{drill.description || drill.tags.join(" · ")}</p></div></div>) : <p className="text-sm text-text-secondary">No drills selected.</p>}</div></div></section>
