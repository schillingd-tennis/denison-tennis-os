...
function HotelForm({ hotel, onCancel, onSaved }: { hotel?: Hotel; onCancel: () => void; onSaved: () => void }) {
  ...
  return <form action={save} className="flex min-h-full flex-col">{hotel ? <input type="hidden" name="id" value={hotel.id}/> : null}<div className="grid flex-1 content-start gap-4 p-5">
    <div className="rounded-card border border-[var(--module-accent)]/15 bg-[var(--module-tint)]/30 p-4"><div className="flex items-center gap-2"><Building2 className="h-4 w-4 text-[var(--module-accent)]"/><p className="text-[10px] font-bold tracking-wider text-[var(--module-accent-text)] uppercase">Hotel record</p></div><p className="mt-1 text-sm text-text-secondary">Keep travel experience and planning notes in one place.</p></div>
