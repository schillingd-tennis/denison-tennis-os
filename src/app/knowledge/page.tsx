...
import { KNOWLEDGE_HOTELS_ROUTE, KNOWLEDGE_OFFICIALS_ROUTE } from "@/lib/module-routes";
  ...
  return (
    <ModulePageShell title="Resources" subtitle="A shared library of team travel, playbooks, resources, and documentation.">
      <div className="grid gap-3">
        ...
        <Link href={KNOWLEDGE_OFFICIALS_ROUTE} className="group flex items-center justify-between rounded-card border border-border bg-surface p-5 shadow-[0_8px_24px_rgba(17,24,39,0.04)] transition-colors hover:border-[var(--module-accent)]/35 hover:bg-[var(--module-tint)]/30">
          <span className="flex items-center gap-4"><span className="flex h-11 w-11 items-center justify-center rounded-card bg-[var(--module-tint)] text-[var(--module-accent)]"><ClipboardList className="h-5 w-5"/></span><span><strong className="block text-base text-text-primary">Officials List</strong><span className="mt-1 block text-sm text-text-secondary">Rate, contact, and assign officials for matches and events.</span></span></span><ChevronRight className="h-5 w-5 text-text-secondary transition-transform group-hover:translate-x-0.5"/>
        </Link>
      </div>
    </ModulePageShell>
  );
