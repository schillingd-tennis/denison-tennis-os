/**
 * Denison UTR Capture — Chrome MV3 service worker.
 * Fetches UTR data via content script; POSTs tennis JSON only to Denison OS.
 */
...
async function fetchFromUtrTab(tab) {
  ...
  let response;
  try {
    response = await chrome.tabs.sendMessage(tab.id, { type: "CAPTURE_UTR_PAGE" });
  } catch {
    throw captureError(
      "UTR_FETCH_FAILED",
...
function formatSummary(result) {
  ...
  return [
    result.recruitName,
    `${o.found} matches read`,
    `${o.crossSourceMatched} matched existing`,
    `${o.savedAsNew} new`,
...
async function notify(title, message) {
  try {
    await chrome.notifications.create(`denison-utr-${Date.now()}`, {
      type: "basic",
...
chrome.action.onClicked.addListener(() => {
  runCapture().catch(async (error) => {
    ...
    await notify("Denison UTR Capture failed", `${code}: ${message}`);
  });
});
